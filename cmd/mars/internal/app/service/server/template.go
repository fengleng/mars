...
var serverRegisterTemplate = `
func init() {
	{{.LowerService}}.Register{{.Service}}Server(gRPCServer, service.New{{.Service}}Service())
	{{.LowerService}}.Register{{.Service}}HTTPServer(httpServer, service.New{{.Service}}Service())
}
`

var MethodTemplate = `{{- $s1 := "google.protobuf.Empty" }}
{{ range .Methods }}
{{- if eq .Type 1 }}
