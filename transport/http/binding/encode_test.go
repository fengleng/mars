...
import (
	...
	"github.com/fengleng/mars/internal/testdata/binding"
)
...
func TestProtoPath(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			request:      &binding.HelloRequest{Name: "go", Sub: &binding.Sub{Name: "mars"}},
			needQuery:    true,
			want:         "http://helloworld.Greeter/helloworld/go/sub?sub.naming=mars",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/sub/{sub.naming}",
			request:      &binding.HelloRequest{Sub: &binding.Sub{Name: "mars"}, UpdateMask: &fieldmaskpb.FieldMask{Paths: []string{"name", "sub.naming"}}},
			needQuery:    false,
			want:         "http://helloworld.Greeter/helloworld/sub/mars?updateMask=name,sub.naming",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/sub/[{sub.naming}]",
			request:      &binding.HelloRequest{Sub: &binding.Sub{Name: "mars"}},
			needQuery:    false,
			want:         "http://helloworld.Greeter/helloworld/sub/[mars]",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/[{name}]/sub/[{sub.naming}]",
			request:      &binding.HelloRequest{Name: "test", Sub: &binding.Sub{Name: "mars"}},
			needQuery:    false,
			want:         "http://helloworld.Greeter/helloworld/[test]/sub/[mars]",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/[{}]/sub/[{sub.naming}]",
			request:      &binding.HelloRequest{Sub: &binding.Sub{Name: "mars"}},
		...
		},
		{
			...
			request:      &binding.HelloRequest{Name: "test", Sub: &binding.Sub{Name: "mars"}},
			needQuery:    false,
			want:         "http://helloworld.Greeter/helloworld/{[name]}/[mars]",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/{}/[]/[{sub.naming}]",
			request:      &binding.HelloRequest{Sub: &binding.Sub{Name: "mars"}},
			needQuery:    false,
			want:         "http://helloworld.Greeter/helloworld/{}/[]/[mars]",
		},
	}
